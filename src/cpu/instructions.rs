...
pub fn execute(&mut self, mmu: &mut Mmu) {
        ...
        let instruction = if self.instruction_t_cycles_remaining == 0 {
            ...
            match self.current_instruction {
                ...
                // 
                // Eventually, ALL multi-cycle instructions should handle each cycle
                // explicitly.
        ...
        match instruction {
            0x00 => (),                                  // NOP
            0x01 => self.ld_r16_n16(R16::BC, mmu),       // LD BC, n16
            0x02 => self.ld_at_r16_a(R16::BC, mmu),      // LD BC, A
            0x03 => self.inc_r16(R16::BC),               // INC BC
            0x04 => self.alu_r8(AluUnary::Inc, R8::B),   // INC B
            0x05 => self.alu_r8(AluUnary::Dec, R8::B),   // DEC B
            0x06 => self.ld_r8_n8(R8::B, mmu),           // LD B, n8
            0x07 => self.rlca(),                         // RLCA
            0x08 => self.ld_at_n16_sp(mmu),              // LD [n16], SP
            0x09 => self.add_hl_r16(R16::BC),            // ADD HL, BC
            0x0A => self.ld_a_at_r16(R16::BC, mmu),      // LD A, [BC]
            0x0B => self.dec_r16(R16::BC),               // DEC BC
            0x0C => self.alu_r8(AluUnary::Inc, R8::C),   // INC C
            0x0D => self.alu_r8(AluUnary::Dec, R8::C),   // DEC C
            0x0E => self.ld_r8_n8(R8::C, mmu),           // LD C, n8
            0x0F => self.rrca(),                         // RRCA

            0x10 => self.stop(),                         // STOP n8
            0x11 => self.ld_r16_n16(R16::DE, mmu),       // LD DE, n16
            0x12 => self.ld_at_r16_a(R16::DE, mmu),      // LD DE, A
            0x13 => self.inc_r16(R16::DE),               // INC DE
            0x14 => self.alu_r8(AluUnary::Inc, R8::D),   // INC D
            0x15 => self.alu_r8(AluUnary::Dec, R8::D),   // DEC D
            0x16 => self.ld_r8_n8(R8::D, mmu),           // LD D, n8
            0x17 => self.rla(),                          // RLA
            0x18 => self.jr_e8(mmu),                     // JR e8
            0x19 => self.add_hl_r16(R16::DE),            // ADD HL, DE
            0x1A => self.ld_a_at_r16(R16::DE, mmu),      // LD A, [DE]
            0x1B => self.dec_r16(R16::DE),               // DEC DE
            0x1C => self.alu_r8(AluUnary::Inc, R8::E),   // INC E
            0x1D => self.alu_r8(AluUnary::Dec, R8::E),   // DEC E
            0x1E => self.ld_r8_n8(R8::E, mmu),           // LD E, n8
            0x1F => self.rra(),                          // RRA

            0x20 => self.jr_cc_e8(Flag::Z, false, mmu),   // JR NZ, e8
            0x21 => self.ld_r16_n16(R16::HL, mmu),        // LD HL, n16
            0x22 => self.ld_at_hli_a(mmu),                // LD [HL+], A
            0x23 => self.inc_r16(R16::HL),                // INC HL
            0x24 => self.alu_r8(AluUnary::Inc, R8::H),    // INC H
            0x25 => self.alu_r8(AluUnary::Dec, R8::H),    // DEC H
            0x26 => self.ld_r8_n8(R8::H, mmu),            // LD H, n8
            0x27 => self.daa(),                           // DAA
            0x28 => self.jr_cc_e8(Flag::Z, true, mmu),    // JR Z e8
            0x29 => self.add_hl_r16(R16::HL),             // ADD HL, HL
            0x2A => self.ld_a_at_hli(mmu),                // LD A, [HL+]
            0x2B => self.dec_r16(R16::HL),                // DEC HL
            0x2C => self.alu_r8(AluUnary::Inc, R8::L),    // INC L
            0x2D => self.alu_r8(AluUnary::Dec, R8::L),    // DEC L
            0x2E => self.ld_r8_n8(R8::L, mmu),            // LD L, n8
            0x2F => self.cpl(),                           // CPL

            0x30 => self.jr_cc_e8(Flag::C, false, mmu),   // JR NC, e8
            0x31 => self.ld_r16_n16(R16::SP, mmu),        // LD SP, n16
            0x32 => self.ld_at_hld_a(mmu),                // LD [HL-], A
            0x33 => self.inc_r16(R16::SP),                // INC SP
            0x34 => self.alu_at_hl(AluUnary::Inc, mmu),   // INC [HL]
            0x35 => self.alu_at_hl(AluUnary::Dec, mmu),   // DEC [HL]
            0x36 => self.ld_at_hl_n8(mmu),                // LD [HL], n8
            0x37 => self.scf(),                           // SCF
            0x38 => self.jr_cc_e8(Flag::C, true, mmu),    // JR C, e8
            0x39 => self.add_hl_r16(R16::SP),             // ADD HL, SP
            0x3A => self.ld_a_at_hld(mmu),                // LD A, [HL-]
            0x3B => self.dec_r16(R16::SP),                // DEC SP
            0x3C => self.alu_r8(AluUnary::Inc, R8::A),    // INC A
            0x3D => self.alu_r8(AluUnary::Dec, R8::A),    // DEC A
            0x3E => self.ld_r8_n8(R8::A, mmu),            // LD A, n8
            0x3F => self.ccf(),                           // CCF
            ...
            0x46 => self.ld_r8_at_hl(R8::B, mmu),   // LD B, [HL]
            ...
            0x4E => self.ld_r8_at_hl(R8::C, mmu),   // LD C, [HL]
            ...
            0x56 => self.ld_r8_at_hl(R8::D, mmu),   // LD D, [HL]
            ...
            0x5E => self.ld_r8_at_hl(R8::E, mmu),   // LD E, [HL]
            ...
            0x66 => self.ld_r8_at_hl(R8::H, mmu),   // LD H, [HL]
            ...
            0x6E => self.ld_r8_at_hl(R8::L, mmu),   // LD L, [HL]
            0x6F => self.ld_r8_r8(R8::L, R8::A),    // LD L, A

            0x70 => self.ld_at_hl_r8(R8::B, mmu),   // LD [HL], B
            0x71 => self.ld_at_hl_r8(R8::C, mmu),   // LD [HL], C
            0x72 => self.ld_at_hl_r8(R8::D, mmu),   // LD [HL], D
            0x73 => self.ld_at_hl_r8(R8::E, mmu),   // LD [HL], E
            0x74 => self.ld_at_hl_r8(R8::H, mmu),   // LD [HL], H
            0x75 => self.ld_at_hl_r8(R8::L, mmu),   // LD [HL], L
            0x76 => self.halt(mmu),                 // HALT
            0x77 => self.ld_at_hl_r8(R8::A, mmu),   // LD [HL], A
            ...
            0x7E => self.ld_r8_at_hl(R8::A, mmu),   // LD A, [HL]
            ...
            0x86 => self.alu_a_at_hl(AluBinary::Add, mmu),   // ADD A, [HL]
            ...
            0x8E => self.alu_a_at_hl(AluBinary::Adc, mmu),   // ADC A, [HL]
            ...
            0x96 => self.alu_a_at_hl(AluBinary::Sub, mmu),   // SUB A, [HL]
            ...
            0x9E => self.alu_a_at_hl(AluBinary::Sbc, mmu),   // SBC A, [HL]
            ...
            0xA6 => self.alu_a_at_hl(AluBinary::And, mmu),   // AND A, [HL]
            ...
            0xAE => self.alu_a_at_hl(AluBinary::Xor, mmu),   // XOR A, [HL]
            ...
            0xB6 => self.alu_a_at_hl(AluBinary::Or, mmu),   // OR A, [HL]
            ...
            0xBE => self.alu_a_at_hl(AluBinary::Cp, mmu),   // CP A, [HL]
            0xBF => self.alu_a_r8(AluBinary::Cp, R8::A),    // CP A, A

            0xC0 => self.ret_cc(Flag::Z, false, mmu),        // RET NZ
            0xC1 => self.pop_r16(R16::BC, mmu),              // POP BC
            0xC2 => self.jp_cc_a16(Flag::Z, false, mmu),     // JP NZ
            0xC3 => self.jp_a16(mmu),                        // JP a16
            0xC4 => self.call_cc_a16(Flag::Z, false, mmu),   // CALL NZ, a16
            0xC5 => self.push_r16(R16::BC, mmu),             // PUSH BC
            0xC6 => self.alu_a_n8(AluBinary::Add, mmu),      // ADD A, n8
            0xC7 => self.rst_vec(0x00, mmu),                 // RST $00
            0xC8 => self.ret_cc(Flag::Z, true, mmu),         // RET Z
            0xC9 => self.ret(mmu),                           // RET
            0xCA => self.jp_cc_a16(Flag::Z, true, mmu),      // JP Z, a16
            0xCB => self.prefixed_instruction_mode = true,   // PREFIX
            0xCC => self.call_cc_a16(Flag::Z, true, mmu),    // CALL Z, a16
            0xCD => self.call_a16(mmu),                      // CALL a16
            0xCE => self.alu_a_n8(AluBinary::Adc, mmu),      // ADC A, n8
            0xCF => self.rst_vec(0x08, mmu),                 // RST $08

            0xD0 => self.ret_cc(Flag::C, false, mmu),        // RET NC
            0xD1 => self.pop_r16(R16::DE, mmu),              // POP DE
            0xD2 => self.jp_cc_a16(Flag::C, false, mmu),     // JP NC, a16
            0xD3 => panic!("Invalid Instruction"),           // ---
            0xD4 => self.call_cc_a16(Flag::C, false, mmu),   // CALL NC, a16
            0xD5 => self.push_r16(R16::DE, mmu),             // PUSH DE
            0xD6 => self.alu_a_n8(AluBinary::Sub, mmu),      // SUB A, n8
            0xD7 => self.rst_vec(0x10, mmu),                 // RST $10
            0xD8 => self.ret_cc(Flag::C, true, mmu),         // RET C
            0xD9 => self.reti(mmu),                          // RETI
            0xDA => self.jp_cc_a16(Flag::C, true, mmu),      // JP C, a16
            0xDB => panic!("Invalid Instruction"),           // ---
            0xDC => self.call_cc_a16(Flag::C, true, mmu),    // CALL C, a16
            0xDD => panic!("Invalid Instruction"),           // ---
            0xDE => self.alu_a_n8(AluBinary::Sbc, mmu),      // SBC A, n8
            0xDF => self.rst_vec(0x18, mmu),                 // RST $18

            0xE0 => self.ldh_at_a8_a(mmu),                // LDH [a8], A
            0xE1 => self.pop_r16(R16::HL, mmu),           // POP HL
            0xE2 => self.ldh_c_a(mmu),                    // LDH [C], A
            0xE3 => panic!("Invalid Instruction"),        // ---
            0xE4 => panic!("Invalid Instruction"),        // ---
            0xE5 => self.push_r16(R16::HL, mmu),          // PUSH HL
            0xE6 => self.alu_a_n8(AluBinary::And, mmu),   // AND A, N8
            0xE7 => self.rst_vec(0x20, mmu),              // RST $20
            0xE8 => self.add_sp_e8(mmu),                  // ADD SP, e8
            0xE9 => self.jp_hl(),                         // JP HL
            0xEA => self.ld_at_a16_a(mmu),                // LD [a16], A
            0xEB => panic!("Invalid Instruction"),        // ---
            0xEC => panic!("Invalid Instruction"),        // ---
            0xED => panic!("Invalid Instruction"),        // ---
            0xEE => self.alu_a_n8(AluBinary::Xor, mmu),   // XOR A, n8
            0xEF => self.rst_vec(0x28, mmu),              // RST $28

            0xF0 => self.ldh_a_at_a8(mmu),               // LDH A, [a8]
            0xF1 => self.pop_r16(R16::AF, mmu),          // POP AF
            0xF2 => self.ldh_a_at_c(mmu),                // LDH A, [C]
            0xF3 => self.di(),                           // DI
            0xF4 => panic!("Invalid Instruction"),       // ---
            0xF5 => self.push_r16(R16::AF, mmu),         // PUSH AF
            0xF6 => self.alu_a_n8(AluBinary::Or, mmu),   // OR A, n8
            0xF7 => self.rst_vec(0x30, mmu),             // RST $30
            0xF8 => self.ld_hl_sp_e8(mmu),               // LD HL, SP + e8
            0xF9 => self.ld_sp_hl(),                     // LD SP, HL
            0xFA => self.ld_a_at_a16(mmu),               // LD A, [a16]
            0xFB => self.ei(),                           // EI
            0xFC => panic!("Invalid Instruction"),       // ---
            0xFD => panic!("Invalid Instruction"),       // ---
            0xFE => self.alu_a_n8(AluBinary::Cp, mmu),   // CP A, n8
            0xFF => self.rst_vec(0x38, mmu),             // RST $38
        }
    }

    pub fn execute_prefixed(&mut self, mmu: &mut Mmu) {
        // todo! Move this under "Fetch instruction"
        let instruction = if self.current_instruction == 0xCB {
            self.prev_instruction = self.current_instruction;
            self.current_instruction = self.fetch_byte(mmu);
        ...
        match instruction {
            ...
            0x06 => self.bitshift_at_hl(BitshiftOp::Rlc, mmu),   // RLC [HL]
            ...
            0x0E => self.bitshift_at_hl(BitshiftOp::Rrc, mmu),   // RRC [HL]
            ...
            0x16 => self.bitshift_at_hl(BitshiftOp::Rl, mmu),   // RL [HL]
            ...
            0x1E => self.bitshift_at_hl(BitshiftOp::Rr, mmu),   // RR [HL]
            ...
            0x26 => self.bitshift_at_hl(BitshiftOp::Sla, mmu),   // SLA [HL]
            ...
            0x2E => self.bitshift_at_hl(BitshiftOp::Sra, mmu),   // SRA [HL]
            ...
            0x36 => self.bitshift_at_hl(BitshiftOp::Swap, mmu),   // SWAP [HL]
            ...
            0x3E => self.bitshift_at_hl(BitshiftOp::Srl, mmu),    // SRL [HL]
            ...
            0x46 => self.bitflag_u3_at_hl(BitflagOp::Bit, 0, mmu),   // BIT 0, [HL]
            ...
            0x4E => self.bitflag_u3_at_hl(BitflagOp::Bit, 1, mmu),   // BIT 1, [HL]
            ...
            0x56 => self.bitflag_u3_at_hl(BitflagOp::Bit, 2, mmu),   // BIT 2, [HL]
            ...
            0x5E => self.bitflag_u3_at_hl(BitflagOp::Bit, 3, mmu),   // BIT 3, [HL]
            ...
            0x66 => self.bitflag_u3_at_hl(BitflagOp::Bit, 4, mmu),   // BIT 4, [HL]
            ...
            0x6E => self.bitflag_u3_at_hl(BitflagOp::Bit, 5, mmu),   // BIT 5, [HL]
            ...
            0x76 => self.bitflag_u3_at_hl(BitflagOp::Bit, 6, mmu),   // BIT 6, [HL]
            ...
            0x7E => self.bitflag_u3_at_hl(BitflagOp::Bit, 7, mmu),   // BIT 7, [HL]
            ...
            0x86 => self.bitflag_u3_at_hl(BitflagOp::Res, 0, mmu),   // RES 0, [HL]
            ...
            0x8E => self.bitflag_u3_at_hl(BitflagOp::Res, 1, mmu),   // RES 1, [HL]
            ...
            0x96 => self.bitflag_u3_at_hl(BitflagOp::Res, 2, mmu),   // RES 2, [HL]
            ...
            0x9E => self.bitflag_u3_at_hl(BitflagOp::Res, 3, mmu),   // RES 3, [HL]
            ...
            0xA6 => self.bitflag_u3_at_hl(BitflagOp::Res, 4, mmu),   // RES 4, [HL]
            ...
            0xAE => self.bitflag_u3_at_hl(BitflagOp::Res, 5, mmu),   // RES 5, [HL]
            ...
            0xB6 => self.bitflag_u3_at_hl(BitflagOp::Res, 6, mmu),   // RES 6, [HL]
            ...
            0xBE => self.bitflag_u3_at_hl(BitflagOp::Res, 7, mmu),   // RES 7, [HL]
            ...
            0xC6 => self.bitflag_u3_at_hl(BitflagOp::Set, 0, mmu),   // SET 0, [HL]
            ...
            0xCE => self.bitflag_u3_at_hl(BitflagOp::Set, 1, mmu),   // SET 1, [HL]
            ...
            0xD6 => self.bitflag_u3_at_hl(BitflagOp::Set, 2, mmu),   // SET 2, [HL]
            ...
            0xDE => self.bitflag_u3_at_hl(BitflagOp::Set, 3, mmu),   // SET 3, [HL]
            ...
            0xE6 => self.bitflag_u3_at_hl(BitflagOp::Set, 4, mmu),   // SET 4, [HL]
            ...
            0xEE => self.bitflag_u3_at_hl(BitflagOp::Set, 5, mmu),   // SET 5, [HL]
            ...
            0xF6 => self.bitflag_u3_at_hl(BitflagOp::Set, 6, mmu),   // SET 6, [HL]
            ...
            0xFE => self.bitflag_u3_at_hl(BitflagOp::Set, 7, mmu),   // SET 7, [HL]
